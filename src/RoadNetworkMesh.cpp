...
template<typename T>
std::vector<size_t> get_outline_indices(const std::map<size_t, T>& start_indices, std::size_t num_vertices)
{
...
std::string RoadSignalsMesh::get_road_signal_id(std::size_t vert_idx) const
{
    return get_nearest_lower_val<size_t, std::string>(this->road_signal_start_indices, vert_idx);
}

std::array<size_t, 2> RoadSignalsMesh::get_idx_interval_signal(std::size_t vert_idx) const
{
